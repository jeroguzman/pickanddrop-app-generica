...
const Deliveries = ({ navigation }) => {
  ...
  useEffect(() => {
    const reloadData = navigation.addListener('focus', () => {
      setLoading(true);
      axios.get(`${url}/couriers/courier-day-orders`, {
        params: {
          courier_id: ids,
        },
        headers: {
      ...
      .then((response) => {
        setLoading(false);
        setLastServices(response.data.data);
      })
      .catch((error) => {
        setLoading(false);
        console.log(error);
  ...
  return (
          <View style={{ gap: 12, padding: 12, backgroundColor: colors.background }}>
            {/* Header */}
              <View style={{ paddingHorizontal: 24, flexDirection: 'row', alignItems: 'center', gap: 8 }}>
                <Text style={{ fontSize: 32, fontWeight: 'bold', color: '#000' }}>Entregas del día</Text>
              </View>
                <ScrollView
  ...
  >
    ...
    <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
        ...
        <Text
          ...
          numberOfLines={2}
        >
          {item.created_at.split('T')[1].split('.')[0].slice(0, -3)}
        </Text>
      </View>
      ...
      <Text
        ...
        numberOfLines={3}
      >
       <Text style={{fontWeight: 600}}>Dirección de Pick:</Text> {item.business.address}
      </Text>
      <Text
        style={{
          fontSize: 12,
